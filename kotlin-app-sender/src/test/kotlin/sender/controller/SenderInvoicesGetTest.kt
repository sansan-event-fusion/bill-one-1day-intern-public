...
import sender.domain.sender.Sender
...
class SenderInvoicesGetTest {
    ...
    fun before() {
        database.setUpTable(listOf(
            ...
            buildSenderInvoiceOperation(listOf(
                ...
                SenderInvoiceTableFixture()._2枚目(),
                SenderInvoiceTableFixture().copy(sender_invoice_uuid = UUID.randomUUID(), sender_uuid = SenderTableFixture()._2人目().sender_uuid),
            )),
        ))
    }

    @Test
    fun `送付済み請求書一覧の取得`(): Unit =withTestApplication({ module(true, settings) }) {
    ...
    @Test
    fun `もう1人の送付済み請求書一覧の取得`(): Unit =withTestApplication({ module(true, settings) }) {
