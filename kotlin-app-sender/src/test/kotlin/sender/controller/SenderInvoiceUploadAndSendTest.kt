...
class SenderInvoiceUploadAndSendTest {
    ...
    @Test
    fun `送付者アップロードのテスト`(): Unit = withTestApplication({ module(true, settings) }) {

        with(handleRequest(HttpMethod.Post, "/api/sender/sender-invoices") {
            ...
            addHeader("X-App-Sender-UUID", SenderTableFixture().sender_uuid.toString())
            addHeader("X-Cloud-Trace-Context", "430963703d1a5d41fa9f63cda15a214d/0;o=1")
            setBody(boundary, listOf(
                PartData.FileItem({
                    javaClass.classLoader.getResource("pdf/upload.pdf")!!.readBytes().inputStream().asInput()
                }, {}, headersOf(
                ...
                PartData.FormItem(
                    RecipientTableFixture().recipient_uuid.toString(),
                    {},
                    headersOf(
                        HttpHeaders.ContentDisposition, ContentDisposition.Inline
            ...
            withHandle { handle ->
                ...
                senderInvoices.first().let {
                    Assertions.assertThat(it.recipientUUID).isEqualTo(RecipientTableFixture().recipient_uuid)
                    Assertions.assertThat(it.senderUUID).isEqualTo(SenderTableFixture().sender_uuid)
                }

                // Storageにファイルが保存されていることを確認
                Assertions.assertThat(getStorageObject(BlobId.of("sender-test-bucket","sender/${SenderTableFixture().sender_uuid}/sender-invoices/${senderInvoices.first().senderInvoiceUUID}.pdf")).size).isEqualTo(63341)

                // ドメインイベントが発行されたことを確認
                val domainEventRow = DomainEventRepository.getByCallUUID(getCallUUID(), handle)
                domainEventRow[0].apply {
                    Assertions.assertThat(this.domainEventName)
                        .isEqualTo("sender.domain.sender_invoice.SenderInvoiceRegistered")
                    JsonStringAssert.assertThat(this.message).jsonMatches("""
                        {
    ...
    private fun getSenderInvoices(handle:Handle): List<SenderInvoiceRow>{
        ...
        return handle.createQuery(sql)
            .mapTo(SenderInvoiceRow::class.java)
            .list()
    }
